          ...
          {projects.map((project, idx) => {
            ...
            return (
              <motion.div key={project.title} initial={{ opacity: 0, x: isLeft ? -120 : 120, y: 20 }} whileInView={{ opacity: 1, x: 0, y: 0 }} transition={{ duration: 0.8, delay: idx * 0.08 }} viewport={{ once: true }} className={`relative z-10 w-full md:w-5/12 ${isLeft ? 'md:ml-0 md:pl-12' : 'md:ml-auto md:pr-12'}`}>
                ...
                <Card className="max-w-4xl p-0 overflow-hidden group shadow-xl" style={{borderRadius: '14px', background: 'linear-gradient(180deg, rgba(255,255,255,0.02), rgba(255,255,255,0.01))', border: '1px solid rgba(255,255,255,0.06)'}}>
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex gap-4 mt-4">
                      <Button as="a" href={project.github} target="_blank" className="flex items-center gap-2 bg-zinc-800/60 hover:bg-zinc-800/80 btn-smooth text-white">
                        <SiGithub /> GitHub
                      </Button>
                      {project.live && (
                        <Button as="a" href={project.live} target="_blank" className="flex items-center gap-2 bg-gradient-to-r from-indigo-600 to-pink-500 hover:from-indigo-500 hover:to-pink-400 text-white btn-smooth">
                          <FaGlobe /> Live
                        </Button>
                      )}
                    </div>
